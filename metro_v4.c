...
GtkTextBuffer *pTxt_buffer; //Ajout d'un text buffer
...
int main(int argc, char **argv)
{
	...
	GtkWidget *pTxt_view; //Ajout d'un text_view
	...
	//Text view
	pTxt_buffer=gtk_text_buffer_new(NULL);
	pTxt_view=gtk_text_view_new_with_buffer(pTxt_buffer);
	gtk_text_view_set_editable(GTK_TEXT_VIEW(pTxt_view),FALSE);
	gtk_box_pack_start(GTK_BOX(pHBox), pTxt_view, FALSE, FALSE, 0);
	...
	//gtk_text_buffer_insert_at_cursor(pTxt_buffer, "Liste des changements :", -1);
